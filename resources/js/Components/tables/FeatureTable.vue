...
export default {
    ...
    filterData(){
      ...
      return  this.pagi.filter(user => {
        return user.feature_name.includes(this.search)||
          user.period_type.includes(this.search)||
          user.periodicity.includes(this.search)
      })
    },
  },
  ...
  methods:{
    async getAllPaginate(){
      ...
      axios
        .get(`api/featurePagination/${this.pageSize}`)
        .then(response => {
    ...
    nextPage(page, query) {
      ...
      axios
        .get(`api/featurePagination/${page}?page=${query}`)
        .then(response => {
          if (response.status == 200){
            (this.pagi = response.data.data,
            this.pageInfo = response.data.pagination
